<template>
    ...
    <table class="table">
        ...
        <tbody>
            <tr v-for="item in historialHorarios" :key="item.id">
...
export default {
     ...
     async created(){
     try {
      await this.$store.dispatch("getHistorial");
     } catch (error) {
      console.error(error);
     }
    },
    ...
    methods:{
     // ...mapActions(['deleteHorarioHistorial']),
       deleteHisto(item) {
      this.$store.dispatch("deleteHistorial", item);
      } ,
       deleteHistoPermanente(item) {
      this.$store.dispatch("deleteHistorialPermanente", item);
      } 
    },
